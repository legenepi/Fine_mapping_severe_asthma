...
kable(df)
```
Fine-mapping plots for each loci with information on LD, variant annotation, and surrounding genes, can be seen and downloaded below. Credible set variants are identified by the red circle, variant ID is for variants with R2 > 0.95 with respect the leading fine-mapping SNP (variant with highest posterior inclusion probability-PIP).
