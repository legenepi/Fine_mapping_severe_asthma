...
dfclean <- df %>% select(CHROM,ID,GENPOS,ALLELE1,ALLELE0,A1FREQ,LOG10P,BETA,SE,CHISQ,INFO)

dfclean$pval <- 10^(-dfclean$LOG10P)

#select column in order:
dfclean <- dfclean %>% select(ID,CHROM,GENPOS,ALLELE1,ALLELE0,BETA,SE,A1FREQ,pval,CHISQ,INFO)

#rename columns :
colnames(dfclean) <- c("SNP", "CHR", "BP", "ALLELE1", "ALLELE2", "BETA", "SE", "A1FREQ", "P", "CHISQ", "INFO")

#make sure eaf column is numeric:
dfclean$A1FREQ <- as.numeric(dfclean$A1FREQ)

#save gwas all chr results:
write.table(dfclean,paste0("input/",pheno,"_betase_input_mungestat"),quote=FALSE,sep=" ",row.names=FALSE)


#create MAF columns and filtered out for MAF >= 0.01:
dfclean <- dfclean %>% mutate(MAF = ifelse(dfclean$A1FREQ <= 0.5, dfclean$A1FREQ, 1 - dfclean$A1FREQ))
dfclean_MAF001 <- dfclean %>% filter(MAF >= 0.01)

#save gaws results for variants with MAF >= 0.01:
write.table(dfclean_MAF001,paste0("input/maf001_",pheno,"_betase_input_mungestat"),quote=FALSE,sep=" ",row.names=FALSE)